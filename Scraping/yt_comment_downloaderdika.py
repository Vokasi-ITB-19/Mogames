...
def download_comments(youtube_id, sleep=.1):
    if 'liveStreamability' in requests.get(YOUTUBE_VIDEO_URL.format(youtube_id=youtube_id)).text:
        print('LiveStream nih! Kayaknya ga semua comment ke donlot.')
...
def main(argv):
    parser = argparse.ArgumentParser(add_help=False, description=('Donlot komenan Yutub tanpa Yutub API.'))
    parser.add_argument('--help', '-h', action='help', default=argparse.SUPPRESS, help='Nampilin help message dan exit')
    parser.add_argument('--youtubeid', '-y', help='Yutub IDnya lur yang mau di donlot. Contoh: https://www.youtube.com/watch?v=[yg ini lur]')
    parser.add_argument('--output', '-o', help='Nama file JSONnya apa lur?')
    parser.add_argument('--limit', '-l', type=int, help='Kasih limit lah, cape akutu :( ')
    ...
    try:
        ...
        if not youtube_id or not output:
            parser.print_usage()
            raise ValueError('Ngeyel ye, Yutub ID sama nama File nya isi dlu lurr.')

        print('Donlot Komenan yutub untuk video: ', youtube_id)
        count = 0
        with io.open(output, 'w', encoding='utf8') as fp:
            for comment in download_comments(youtube_id):
                ...
                count += 1
                sys.stdout.write('Kedonlot %d komenan\r' % count)
                sys.stdout.flush()
                print(comment_json)
                if limit and count >= limit:
                    break
        print('Telah Terdownload %d Komenan\r' % count)
        print('\nSelesai!')

    except Exception as e:
        print('ERROR BROOO:', str(e))
        sys.exit(1)
